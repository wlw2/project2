...
public class AdminClient  {
	...
	public static void main( String[] args ) throws IOException, InterruptedException
	{
		...
		options.addOption("shutdown",true,"regular shutdown, a string");
		...
		if (cmd.hasOption("password")) {
			String check = cmd.getOptionValue("password");
			clientManager.emit("password", check);

			if (cmd.hasOption("shutdown")) {
				String event_emitted = cmd.getOptionValue("shutdown");
				clientManager.emit("shutdown", event_emitted);
			}
			else if (cmd.hasOption("force")) {
				String event_emitted = cmd.getOptionValue("force");
				clientManager.emit("forceshutdown", event_emitted);
			}
			else if(cmd.hasOption("vader")){
				String event_emitted = cmd.getOptionValue("vader");
				clientManager.emit("vadershutdown", event_emitted);
			}
		}
